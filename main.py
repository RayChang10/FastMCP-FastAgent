...
def start_virtual_interviewer():
    ...
    try:
        ...
        if interviewer_path.exists():
            ...
            # 檢查是否有 run.py
            run_py = interviewer_path / "run.py"
            if run_py.exists():
                # 命令傳入檔名，cwd 指向目錄，避免路徑重複
                subprocess.run(
                    [sys.executable, "run.py"], check=True, cwd=str(interviewer_path)
                )
            else:
        ...
        def run_virtual_interviewer():
            try:
                ...
                if interviewer_path.exists():
                    ...
                    if run_py.exists():
                        # 命令傳入檔名，cwd 指向目錄，避免路徑重複
                        # 移除 check=True 以避免程序崩潰
                        subprocess.run(
                            [sys.executable, "run.py"],
                            cwd=str(interviewer_path),
                        )
                    else:
                        logger.error("❌ virtual_interviewer/run.py 不存在")
                else:
...
def test_database():
    ...
    try:
        # 檢查是否有現有的測試文件
        from test_interview_flow import main as interview_test_main
        from test_question_analysis import main as question_test_main
