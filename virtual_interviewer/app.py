...
class InterviewAPI(Resource):
    ...
    def _transition_state(self, user_id, user_message):
        ...
        # 從 INTRO 轉換到 INTRO_ANALYSIS（完成自我介紹）
            ...
            # 只有當用戶說"介紹完了"時才進入解析階段，字與字數要一模一樣
    ...
    def _process_with_state_controlled_agent(
        ...
        try:
            ...
            if interview_data and any(
                ...
                self._set_user_state(user_id, InterviewState.COMPLETED)
                # 直接返回完成訊息，避免重複處理
                return self._generate_exit_message(interview_data)
    ...
    def _generate_exit_message(self, interview_data=None):
        """生成統一的退出訊息"""
        try:
            # 嘗試使用 Fast Agent 生成總結
...
請開始您的自我介紹：
            ...
            # 檢查是否為結束自我介紹的關鍵字 - 只有"介紹完了"四個字一模一樣才能觸發
            if user_message == "介紹完了":
                # 觸發狀態轉換到自我介紹分析階段
